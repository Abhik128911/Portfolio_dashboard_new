...
// // 🔥 CLEANER FUNCTION FOR TEXT FILES 🔥
...
const DOM = {
  ...
  assignmentContainer: document.getElementById("assignmentContainer"),
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("subjects.json")
    ...
    .catch(err => {
      ...
      DOM.subjectContainer.innerHTML = "<p class='error' style='text-align:center;'>Failed to load subjects. Check console for details.</p>";
    });
});
...
function renderSubjects(data) {
  ...
  DOM.assignmentContainer.classList.add("hidden");
  DOM.backBtn.classList.add("hidden");
  DOM.downloadOptions.classList.add("hidden"); 
  
  DOM.pageTitle.innerText = "🎓 College Lab Repository";
  DOM.pageDesc.innerText = "Select a subject to view assignments";
  DOM.searchInput.placeholder = "Search subjects...";
  ...
  data.forEach(subject => {
    ...
    card.innerHTML = `
      ...
      <button onclick="openSubject('${subject.id}')">View Assignments →</button>
    `;
    DOM.subjectContainer.appendChild(card);
  });
}
...
function openSubject(id) {
  ...
  currentSubjectInfo = subject;
  DOM.pageTitle.innerText = `${subject.icon || '📚'} ${subject.subjectName}`;
  DOM.pageDesc.innerText = "Browse assignments and solutions below";
  DOM.searchInput.placeholder = "Search assignments...";
  DOM.searchInput.value = "";
  
  DOM.subjectContainer.classList.add("hidden");
  DOM.assignmentContainer.classList.remove("hidden");
  ...
  fetch(subject.file)
    ...
    .catch(err => {
      console.error("Error loading assignments:", err);
      DOM.assignmentContainer.innerHTML = `<p class='error' style='text-align:center;'>Failed to load assignments for ${subject.subjectName}.</p>`;
    });
}

function renderAssignments(assignments) {
  DOM.assignmentContainer.innerHTML = "";
  ...
  assignments.forEach((a) => {
    ...
    card.innerHTML = `
      ...
      <button onclick="openModalByTitle('${safeTitle}')">View Solution →</button>
    `;
    DOM.assignmentContainer.appendChild(card);
  });
}
...
// ==========================================
function downloadTXT() {
  if (!currentAssignments || !currentAssignments.length) return alert("No content to save!");
  ...
  currentAssignments.forEach(exp => {
    ...
    if (exp.questions) {
      exp.questions.forEach(q => {
        ...
        expHTML += `
          <div style="margin-top:15px;">
            <h4 style="margin-bottom:5px; color:#334155; border-bottom:1px solid #eee; display:
